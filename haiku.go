...
// 
// // 各ステージの終了を待つ
// func waitFinishStage(ctx context.Context, game *repository.HaikuGame, c chan int) {
// 	defer close(c)
// 
// 	ctx2, cancel := context.WithTimeout(ctx, 120*time.Second)
// 	defer cancel()
// 
// 	it := .Collection("poems").Snapshots(ctx2)
// 	for {
// 		snap, err := it.Next()
...
// 	}
// 
// 	game, poet := haikuRepo.GetGameByPoetID(m.Author.ID)
// 	if game == nil {
// 		return
// 	}
// 
...
// 	poem := haikuRepo.GetNextPoem(poet, game)
// 
// 	poemRune := repository.NewPoemRune(poet.ID, m.Content)
// 	poem.PoemRunes = append(poem.PoemRunes, poemRune)
// 
// 	ctx := context.Background()
